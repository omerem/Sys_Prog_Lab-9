...
int getBinaryFlags(int n);
extern int startup(int argc, char **argv, void (*start)());
void task2c(int argc, char ** argv);#include <stdio.h>
#include <stdbool.h>
#include <string.h>
#include <stdlib.h>
#include <sys/mman.h>
...
int getBinaryFlags(int n);
extern int startup(int argc, char **argv, void (*start)());
void task2c(int argc, char ** argv);
...
void load_phdr(Elf32_Phdr *phdr, int fd){
    ...
    if(phdr->p_type == 1){
        mmap(vaddr, phdr->p_memsz+padding, 
           phdr->p_flags,
...
void task2c(int argc, char ** argv)
{
    ...
    startup(argc, argv, (void (*) ())header->e_entry);
    munmap(map_start, fd_stat.st_size);
}
